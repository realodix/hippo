...
final class FileHandler
{
     ...
     *
     * @param string $path File or directory path to process
     * @param array<string> $ignore Paths to ignore if directory
     * @param \Realodix\Hippo\Enums\Mode $mode Processing mode
     * @param string $cachePath Path to cache file
     */
    public function handle(string $path, array $ignore, Mode $mode, string $cachePath): ProcessingStats
    {
        $stats = ProcessingStats::empty();

        $this->cache->prepareForRun($cachePath, $mode);
        ...
        if (is_file($path)) {
            ...
            $stats = $this->processDirectory($path, $ignore, $mode);
        }
