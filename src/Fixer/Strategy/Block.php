...
final class Block
{
    ...
    private function shouldProcessAllBlocks(string $filePath, int $totalBlocks, array $blocksToProcess, Mode $mode): bool
    {
        ...
        $isNewFile = empty(Arr::get($cacheEntry, 'blocks'));
    ...
    private function processAllBlocks(array $content, Mode $mode)
    {
        ...
        if ($mode === Mode::Partial) {
            ...
            $newBlockHashes = $this->blockHash($newBlocks);
        }
    ...
    private function processChangedBlocks(array $originalBlocks, int $totalBlocks, array $blocksToProcess)
    {
        ...
        $newBlockHashes = $this->blockHash($processedBlocks);
    ...
    private function getBlocksToProcess(string $filePath, array $originalBlocks, Mode $mode): array
    {
        ...
        $cacheEntry = $this->cache->repository()->get($filePath);
        $cachedBlockHashes = Arr::get($cacheEntry, 'blocks');
        $currentBlockHashes = $this->blockHash($originalBlocks);

        // Safeguard for an empty cache. This prevents errors on the subsequent line
        // and ensures shouldProcessAllBlocks() will trigger a full reprocessing.
        if (empty($cachedBlockHashes)) {
        ...
        foreach ($currentBlockHashes as $index => $currentHash) {
            if (Arr::get($cachedBlockHashes, $index) !== $currentHash) {
     ...
     * @return array<string>
     */
    private function blockHash(array $blocks): array
    {
        $hashes = [];
        foreach ($blocks as $i => $block) {
            $hashes[$i] = $this->cache->hash(implode("\n", $block));
        }
